...
def cmd_fetch(args):
    ...
    with http.request(
        "GET", UCD_URL + "/" + args.db_version + "/UCD.zip", preload_content=False
    ) as req, open(zip_path, "wb") as out_file:
...
class UnicodeData:
    def __init__(self, path: pathlib.Path):
        self.source: zipfile.ZipFile | pathlib.Path = (
            zipfile.ZipFile(path) if path.name.endswith(".zip") else path
        )

    @staticmethod
    def remove_comments(line: str) -> list[str] | None:
        parts = line.split(";")
        if len(parts) < 3:
            return None
    ...
    def load_file(
        ...
        *,
        filter: Callable[[str], list[str] | None] = remove_comments,
    ) -> Iterator[list[str]]:
        if isinstance(self.source, zipfile.ZipFile):
            file = io.TextIOWrapper(self.source.open(relpath), "utf-8")
        else:
            file = open(self.source / relpath, "r")
        for line in file:
            if (result := filter(line)) is not None:
                yield result
...
def cmd_make_casefold_data(args):
    udata = UnicodeData(pathlib.Path(args.db_dir) / f"UCD-{args.db_version}.zip")
    ...
    # For each equivalence class, map every member to the next member, or itself if the class is empty
...
def make_make_arrays_recursive(deltas):
    @cache
    def make_arrays_recursive(sizes):
        if len(sizes) == 1:
            arrays = []
            prev_refs = deltas
        else:
            arrays, prev_refs = make_arrays_recursive(sizes[:-1])
            arrays = copy.deepcopy(arrays)
            prev_refs = copy.deepcopy(prev_refs)
        size = sizes[-1]
        blocks = [
            prev_refs[i * size : (i + 1) * size] for i in range(len(prev_refs) // size)
        ]
        blocks_uniques = []
        blocks_refs = []
        for block in blocks:
...
def try_sizes_new(deltas: list[int], num_tables: list[int]):
    max_rune = UTF_MAX + 1
    pow2: list[int] = [2**x for x in range(1, max_rune.bit_length())]
    options: list[tuple[int, list[int]]] = []
    make_arrays = make_make_arrays_recursive(deltas)
    # Exhaustively try bit lengths for each array.
    for array_sizes in map(tuple, itertools.product(pow2, repeat=num_tables)):
        if math.prod(array_sizes) > max_rune:
            continue
        arrays, prev_refs = make_arrays(array_sizes)
        arrays = arrays + [prev_refs]
        # Compute the number of bytes needed to store the resulting arrays.
        out_array_sizes = [
            len(a) * bytewidth(a, signed=i == 0) for i, a in enumerate(arrays)
        ]
        uncompressed_size = sum(out_array_sizes)
        out_csv = (
            [uncompressed_size, len(array_sizes)] + list(array_sizes) + out_array_sizes
        )
        options.append((uncompressed_size, array_sizes))
        print(",".join([str(x) for x in out_csv]), file=sys.stderr)
    lowest = sorted(options)[0]
    print("lowest " + (",".join([str(x) for x in lowest[1]]) + "\n"), file=sys.stderr)


def render_casefold(sizes, deltas):
    arrays = make_arrays(sizes, deltas, {})
    for i, (array, size) in enumerate(
        zip(arrays, sizes + [0x110000 // math.prod(sizes)])
    ):
        bitwidth = bytewidth(array, signed=i == 0) * 8
        inttype = "uint" if i != 0 else "int"
        print(
            f"MN_INTERNAL_DATA mn_{inttype}{bitwidth} re__rune_data_casefold_{i}[] = "
        )
...
def try_sizes(deltas):
    ...
    for size_a in pow2:
        for size_b in pow2:
            ...
            blocks_a = [
                deltas[i * size_a : (i + 1) * size_a] for i in range(max_rune // size_a)
            ]
            blocks_a_uniques = []
            blocks_a_refs = []
            ...
            blocks_b = [
                mapped_refs[i * a_per_b : (i + 1) * a_per_b]
                for i in range(max_rune // size_b)
            ]
            blocks_b_uniques = []
            blocks_b_refs = []
            ...
            uncomp_size = (
                len(a_array) * bytewidth(max(deltas))
                + len(b_array) * bytewidth(max(b_array))
                + len(c_array) * bytewidth(max(c_array))
            )
            dzz_size = (
                len(dzz_encode(a_array))
                + len(dzz_encode(b_array))
                + len(dzz_encode(c_array))
            )
            print(
                "A: %8i B: %8i C: %8i Uncompressed: %8i Compressed: %8i"
                % (size_a, size_b, size_c, uncomp_size, dzz_size)
            )
...
def heuristic_squish(blocks) -> tuple[list[int], list[int]]:
...
if __name__ == "__main__":
    ...
    parse.add_argument(
        "--db-dir", type=str, help="directory to store data in", default="unicode_data"
    )
    parse.add_argument(
        "--db-version", type=str, help="version of database to fetch", default="latest"
    )
    parse.add_argument(
        "--debug",
        action="store_const",
    ...
    parse_cmd_dump = parse_cmds.add_parser(
        "dump_set", help="dump set from UnicodeData.txt"
    )
    parse_cmd_dump.set_defaults(func=cmd_gen_class)
    parse_cmd_dump.add_argument(
        "criteria", type=str, help="filter criteria, name=value separated by commas"
    )
    parse_cmd_dump.add_argument(
        "--format",
        type=str,
    ...
    parse_cmd_dump_vals = parse_cmds.add_parser(
        "make_prop_data", help="make property data arrays"
    )
